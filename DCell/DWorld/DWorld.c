...
PetscErrorCode DWorldCreate( FluidField fluid, DWorld *world )
{
  ...
  w->Simulate = DWorldSimulate_Picard;
...
#undef __FUNCT__
#define __FUNCT__ "DWorldSimulate_Picard"
PetscErrorCode DWorldSimulate_Picard(DWorld w)
{
  ...
  for ( w->ti = 0; w->t < w->tend && w->ti < w->timax; ++w->ti) {
    ...
    ierr = DCellsArrayInitPicard( w->dcells ); CHKERRQ(ierr);
    ierr = DCellsArrayAdvectPicard( w->dcells, w->fluid, w->dt ); CHKERRQ(ierr);
    ierr = DWorldWrite( w, 0); CHKERRQ(ierr);
    int k;
    for (k = 1; k < 50; ++k) {
      ierr = VecZeroEntries(fluid->rhs); CHKERRQ(ierr);
      ierr = DCellsArrayUpdateFluidFieldRHS(w->dcells, w->iim, fluid, w->t ); CHKERRQ(ierr);
      ...
      ierr = DCellsArrayAdvectPicard( w->dcells, w->fluid, w->dt ); CHKERRQ(ierr);
      ierr = DWorldWrite( w, k); CHKERRQ(ierr);
    }
