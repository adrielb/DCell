...
//  ierr = LevelSetInitializeToStar2D(dh,center,radius,0.5*radius, 5,&ls); CHKERRQ(ierr);
  ...
  ierr = LevelSetWriteIrregularNodeList(ls->irregularNodes,-1); CHKERRQ(ierr);
  ...
  ierr = DACreate2d(PETSC_COMM_WORLD,DA_NONPERIODIC,DA_STENCIL_BOX,
              n.x,n.y
              , PETSC_DECIDE,PETSC_DECIDE, dof,1, 0,0, &da); CHKERRQ(ierr);

  Vec vecRHS;
  ierr = DACreateGlobalVector(da,&vecRHS); CHKERRQ(ierr);
