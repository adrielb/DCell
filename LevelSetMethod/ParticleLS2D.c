...
#undef  __FUNCT__
#define __FUNCT__ "ParticleLS_AdvectParticles2D"
PetscErrorCode ParticleLS_AdvectParticles2D(ParticleLS pls, Coor dh, Grid velgrid, PetscReal dt)
{
  ...
  for ( i = 0; i < ArrayLength(pls->particles); ++i) {
     ...
     * Xp = Xp + dt V2
     */
    ierr = InterpolateVelocity2D(0,vel,p->X,&V1); CHKERRQ(ierr);
    X.x = p->X.x + dt/2 * V1.x / dh.x;
    X.y = p->X.y + dt/2 * V1.y / dh.y;

    ierr = InterpolateVelocity2D(0,vel,X, &V2); CHKERRQ(ierr);
    p->X.x += dt * V2.x / dh.x;
    p->X.y += dt * V2.y / dh.y;
...
#undef  __FUNCT__
#define __FUNCT__ "ParticleLS_ErrorCorrection2D"
PetscErrorCode ParticleLS_ErrorCorrection2D( ParticleLS pls, LevelSet ls )
{
        ...
        PetscAbs( dist ) > p->radius ) // by more than its radius
    {
      ...
      for (j = 0; j < 2; ++j) {
        for (i = 0; i < 2; ++i) {
          ...
          if( sign > 0 ) {
            phi_pos[a.y][a.x] = PetscMax( phi_p, phi_pos[a.y][a.x] );
          } else {
            phi_neg[a.y][a.x] = PetscMin( phi_p, phi_neg[a.y][a.x] );
          } // phi+ or phi-
        } // i
      } // j
  ...
  for( m = 0; m < ArrayLength(ls->band); m++ )
  {
    ...
    if( PetscAbs(phi_pos[b->y][b->x]) <=
        PetscAbs(phi_neg[b->y][b->x]) ) {
      phi[b->y][b->x] = phi_pos[b->y][b->x];
    } else {
      phi[b->y][b->x] = phi_neg[b->y][b->x];
    }
  }
